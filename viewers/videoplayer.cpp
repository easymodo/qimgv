...
static void wakeup(void *ctx) {
    ...
    VideoPlayer *videoPlayer = (VideoPlayer *)ctx;
    emit videoPlayer->mpv_events();
}
...
VideoPlayer::VideoPlayer(QWidget *parent) {
    ...
    mpvContainer = new QWidget(this);
    mpvContainer->setAccessibleName("mpvContainer");
    layout = new QVBoxLayout(this);
    layout->addWidget(mpvContainer);
    layout->setContentsMargins(0,0,0,0);
    this->setLayout(layout);
    mpvContainer->setAttribute(Qt::WA_DontCreateNativeAncestors);
    mpvContainer->setAttribute(Qt::WA_NativeWindow);

    // pass all mouse events to parent MainWindow
    mpvContainer->setAttribute(Qt::WA_TransparentForMouseEvents);
    setAttribute(Qt::WA_TransparentForMouseEvents);
    mpvContainer->setFocusPolicy(Qt::NoFocus);
    setFocusPolicy(Qt::NoFocus);

    // If you have a HWND, use: int64_t wid = (intptr_t)hwnd;
    int64_t wid = mpvContainer->winId();
    ...
    connect(this, &VideoPlayer::mpv_events, this, &VideoPlayer::on_mpv_events,
            Qt::QueuedConnection);
...
void VideoPlayer::on_file_open(Clip *clip) {
    if(clip) {
        mClip = clip;
        QString filename = clip->getPath();
        if (mpv) {
...
void VideoPlayer::on_file_close() {
    if(mpv) {
        // TODO: stop playback
    }
}

void VideoPlayer::on_new_window() {
